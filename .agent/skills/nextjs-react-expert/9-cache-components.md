...
> [!IMPORTANT]
> This is a Next.js 15+ (canary/experimental) skill. These features require the `experimental.dynamicIO` flag in `next.config.ts`. Do NOT apply to stable Next.js 15 without explicitly enabling the flag.

## Core Philosophy
Next.js 15+ canary marks the transition from "Segment-level caching" to "Component-level caching". We no longer rely on `export const revalidate = 3600`. Instead, we use granular directives and profiles.
...
Next.js 15+ canary enables PPR via `experimental.ppr = true` in `next.config.ts`. 
