...
def detect_test_framework(project_path: Path) -> dict:
    """Detect test framework and commands (Bun-aware, Vitest preferred)."""
    result = {
        "type": "unknown",
        "framework": None,
        "cmd": None,
    ...
    if package_json.exists():
        ...
        try:
            ...
            # Detect runtime: Bun preferred when bun.lockb present
            is_bun = (project_path / "bun.lockb").exists()

            # Priority: bun test > vitest > jest > npm test
            if is_bun and "vitest" not in deps and "jest" not in deps:
                # Bun has a built-in Jest-compatible test runner
                result["framework"] = "bun test"
                result["cmd"] = ["bun", "test"]
                result["coverage_cmd"] = ["bun", "test", "--coverage"]
                ...
                # Fallback: project defines its own test script
                runner = "bun" if is_bun else "npm"
                result["framework"] = f"{runner} test (script)"
                result["cmd"] = [runner, "test"]

                # Still detect framework for coverage
                if "vitest" in deps:
                    result["coverage_cmd"] = ["npx", "vitest", "run", "--coverage"]
                elif "jest" in deps:
                    result["coverage_cmd"] = ["npx", "jest", "--coverage"]
