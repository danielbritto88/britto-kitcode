...
def detect_project_type(project_path: Path) -> dict:
    """Detect project type and available linters (Biome/Bun-aware)."""
    result = {
        "type": "unknown",
        "linters": []
    }
    ...
    if package_json.exists():
        ...
        try:
            ...
            # Detect runtime: Bun preferred when bun.lockb exists
            is_bun = (project_path / "bun.lockb").exists()
            runner = "bun" if is_bun else "npm"

            # --- Lint ---
            if "lint" in scripts:
                # Use bun run or npm run based on runtime
            ...
            if "typecheck" in scripts or "type-check" in scripts:
                key = "typecheck" if "typecheck" in scripts else "type-check"
                result["linters"].append({
                    "name": "tsc (via script)",
                    "cmd": [runner, "run", key]
                })
