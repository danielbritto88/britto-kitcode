...
1. **Read** `.context/CODEBASE.md` → Check **OS** field (Windows/macOS/Linux)
2. **Read** any existing plan files in `.context/`
...
7. **Create `.context/{task-slug}.md` (MANDATORY for PLANNING mode)**
...
5. **Location:** `.context/` (project context folder)
...
## 5-PHASE WORKFLOW (SDD + BMAD)

### Phase Overview

| Phase | Name | Focus | Output | Code? |
|-------|------|-------|--------|-------|
| 0 | **SPEC** | Define WHAT (behavior, contracts) | `{task-slug}-spec.md` | ❌ NO |
...
> **Flow:** SPEC → USER APPROVAL → ANALYSIS → PLANNING → USER APPROVAL → SOLUTIONING → DESIGN APPROVAL → IMPLEMENTATION → VERIFICATION
...
## Postconditions
- [what is guaranteed after this runs]

## Invariants
- [what never changes regardless of input]

## Edge Cases
- [boundary values, empty inputs, error states]
...
**Rules:**
- SPEC must be approved by user BEFORE creating the plan file
- When a requirement changes → edit SPEC → regenerate affected tasks
- SPEC is the anchor of intent — it persists with the code
...
| **PLANNING**| "build", "refactor", "create"| SPEC → Task Breakdown + Dependencies| ✅ YES |
...
> For each task, indicate the best agent AND the best skill from the project to implement it.
...
### Create Plan File (DYNAMIC NAMING)
...
**Plan Storage (For PLANNING Mode):** `.context/{task-slug}.md`
...
```
[IF PLANNING MODE]
[OK] Plan file written to .context/{slug}.md
[OK] Read .context/{slug}.md returns content
...
| 8 | **SPEC First** | SPEC approved before plan | Requirement anchor |
